...
class SmartSubtitleSegmenter:
    ...
    2. Strict Syntactic phrase protection:
       ...
       - Never splits a compound verb or imperative phrase (e.g. 'hãy tuyên sấm' stays together).
       - Never creates orphan lines (single short word on a new line).
    3. Punctuation & Dialogue Boundary Splitting:
       - Dialogue introductions (e.g. 'phán với tôi rằng :', 'nói rằng :') are automatically sealed as their own clean screen.
       - Sentence completions (. ? ! …) seal the screen cleanly.
    4. Knuth-Plass DP Optimal 2-line balancing.
    """
    ...
    def evaluate_line_split_penalty(cls, line1_words: List[str], line2_words: List[str], max_cpl: int = 40) -> float:
        ...
        last_word_clean = line1_words[-1].lower().strip(".,:;!?\"“”'()[]")
        if last_word_clean in cls.DANGLING_END_WORDS:
            penalty += 700.0  # Reject 'hãy', 'các', 'với', 'rằng' at end of line 1

        # First word of line 2 being loose punctuation
        first_word_clean = line2_words[0].lower().strip(".,:;!?\"“”'()[]")
        if first_word_clean in {",", ";", ":", "."}:
            penalty += 1000.0
        ...
        if last_word_clean == "rằng" or line1_words[-1].endswith("rằng:") or line1_words[-1].endswith("rằng,"):
            penalty -= 250.0
    ...
    def format_line_breaks(cls, text: str, max_cpl: int = 40, max_lines: int = 2) -> str:
        ...
        Guarantees maximum 2 lines, no dangling words, no orphan words.
        """
        # Normalize double colons / spacing
    ...
    def find_best_clause_split(cls, words: List[Dict[str, Any]], max_chars_per_screen: int = 80) -> int:
        ...
        for i in range(2, len(words) - 1):
            ...
            # Anti-dangling penalty: Never split right after auxiliary or determiner
            w_prev_clean = w_prev.lower().strip(".,:;!?\"“”")
            if w_prev_clean in cls.DANGLING_END_WORDS:
                score -= 700.0
    ...
    def segment_words(
        ...
        for i, word_item in enumerate(clean_words):
            ...
            # 4. Exceeds max screen capacity (80 chars) or max duration
                ...
                # Do NOT split if only 1 word remains until the end of the clean_words stream
                remaining_words = len(clean_words) - i
                if remaining_words > 1:
                    should_split = True

            if should_split:
                # Check if the block we are about to seal should be split at a natural clause
